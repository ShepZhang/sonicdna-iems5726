...
\begin{sloppypar}
\noindent Per the course requirement, the trained model artifacts are
\emph{not} bundled in the submission ZIP. They are published in the
public source repository at
\url{https://github.com/ShepZhang/sonicdna-iems5726} (under
\path{source_code/models/}) and can also be regenerated from the Kaggle
dataset in roughly two minutes by running
\path{pipeline/preprocess.py}. See \path{application/MODELS.md} in the
ZIP for retrieval instructions.
\end{sloppypar}
...
\begin{itemize}
  ...
  \item Local run: \path{docker build -t sonicdna .} followed by
        \path{docker run -p 8501:8501 sonicdna}, then open
        \texttt{http://localhost:8501}. The full Streamlit experience
        (audio-reactive MoodPrint, Tone.js BGM, parallel universes,
        90k-track scatter map) runs entirely offline once the image is
        built.
\end{itemize}
...
% Auto-pick: if `veriguide.pdf` (signed receipt) is present in this
% directory, embed it as a full-page figure. Otherwise show a red
% placeholder so the public GitHub copy still compiles cleanly without
% leaking the signed copy into the public repo.
\IfFileExists{veriguide.pdf}{%
  \begin{center}
    \includegraphics[width=0.92\linewidth,
                     height=0.85\textheight,
                     keepaspectratio]{veriguide.pdf}
  \end{center}
}{%
  \begin{center}
    \fbox{\parbox{0.82\linewidth}{\centering
      \textcolor{red}{\textbf{[ INSERT VERIGUIDE RECEIPT HERE ]}}\\[6pt]
      \small The signed VeriGuide receipt is kept out of the public
      source repository for privacy. Drop the signed receipt as
      \texttt{report\_tex/veriguide.pdf} locally, and this section
      will be replaced by the receipt automatically on the next
      \texttt{pdflatex} run.}}
  \end{center}
}
